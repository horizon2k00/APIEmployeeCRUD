...
export class UpdateModule {
    ...
    static argBulkUpdate(req: { route?: { path: string; }; body?: any; query?: any; }, res: { send: (arg0: string | any[]) => void; }, arg: string) {
        if (arg === 's') { arg = 'salary'; }
        ...
        employees.map((emp: employee) => {
            ids.map((id: number) => {
                if (emp.empId === id) {
                    const change :changeLog = UpdateModule.createChangeLog(emp, changeLog);
    ...
    static update(req: { params: { id: string; }; body: { email: any; name: any; age: any; position: any; department: any; salary: any; privilege: any; rating: any; }; }, res: { index: number; send: (arg0: string) => void; }) {
        const emp = FilterData.getEmp();
        const changeLog = FilterData.getChangeLogs();
        let change:changeLog = {id:undefined, empId: parseInt(req.params.id), createdAt: Date.now(), before: {}, after: {},updatedAt:undefined };
    ...
    static passAuth(req: { jwtPayload: any; body: any; }, res: { send: (arg0: string) => void; }, next: () => void) {
    ...
    static authorizeUser(req: { jwtPayload: any; }, res: { index: number; send: (arg0: string) => void; }, next: () => void) {
    ...
    static createChangeLog(emp: employee, changeLog: string | any[]) {
        ...
        const change:changeLog = {id, empId: emp.empId, createdAt: Date.now(), before: {}, after: {}, updatedAt:undefined};
        return change;
    }
    
    //Middleware to check if wmp with requested email exists
    static verifyIndexByEmail(req: { body: { email: any; }; }, res: { employees: employee[]; index: number; }, next: any) {
    ...
    static updatePwd(req: { body: { oldPassword: any; password: any; }; }, res: { employees: employee[]; index: number; send: (arg0: string) => void; }) {
        const changeLog = FilterData.getChangeLogs();
