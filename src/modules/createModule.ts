...
import { employee,Request,Response,NextFunction } from './typeDefinitions.js';

export class CreateModule {
    
    static fs = require('fs');
    static path = require('path');
    static datapath:string = CreateModule.path.join(__dirname, '../DATA/data.json');
    static bcrypt = require('bcrypt');

    static exists(req:Request, res:Response, next: NextFunction) {
    ...
    static createEntry(req:Request, res: Response) {
        const employees:employee[] = FilterData.getEmp();
        const index:number = Verify.findEmp(employees, 'email', req.body.email);
        ...
        if (index === -1) {
            ...
            CreateModule.bcrypt.hash(req.body.password, 5).then((hash: string) => {
